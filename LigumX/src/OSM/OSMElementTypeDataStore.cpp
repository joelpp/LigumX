...
{
{ "Data", PIDX_Data, offsetof(OSMElementTypeDataStore, m_Data), 0, LXType_stdvector, sizeof(std::vector<OSMElementTypeData*>), LXType_stdvector, false, LXType_OSMElementTypeData, true, 0, 0, 0, 0,}, 
};
...
void OSMElementTypeDataStore::Clone(LXObject* otherObj)
{
	super::Clone(otherObj);
	OSMElementTypeDataStore* other = (OSMElementTypeDataStore*) otherObj;
...
#define AddAttribute(key, value) data->GetAttributes().push_back(new OSMAttribute(#key, #value));
#define AddSingleAttribute(key) data->GetAttributes().push_back(new OSMAttribute(#key, ""));
